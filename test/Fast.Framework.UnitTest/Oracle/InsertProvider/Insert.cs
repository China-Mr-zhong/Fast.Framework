...
namespace Fast.Framework.UnitTest.Oracle.InsertProvider
{
    ...
    public class Insert : TestBase
    {
        ...
        {
            //注意:需要使用As方法显示指定表名称
            var insert = db.Insert(InsertDataSource.GetAnonymousObj()).As("Product").IgnoreColumns("ProductId");
            var sql = insert.ToSqlString();
            var result = insert.Exceute();
        ...
        {
            //注意:需要使用As方法显示指定表名称
            var insert = db.Insert(InsertDataSource.GetAnonymousObjList()).As("Product").IgnoreColumns("ProductId");
            var sql = insert.ToSqlString();
            var result = insert.Exceute();
        ...
        {
            //注意:需要使用As方法显示指定表名称
            var result = db.Insert(InsertDataSource.GetKeyValues()).As("Product").Exceute();
        ...
        {
            //注意:需要使用As方法显示指定表名称
            var result = db.Insert(InsertDataSource.GetKeyValuesList()).As("Product").IgnoreColumns("ProductId").Exceute();
